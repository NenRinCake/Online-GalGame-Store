...
<%
    ...
    Bought[] boughts = null;
    try{
        boughts = BoughtServlet.boughtGoods(username);
        cardCount = boughts.length;
    }catch (Exception e){
        e.printStackTrace();
    }
    ...
    <tr>
        ...
        <td>
            <div class="card" style="position: relative; width: 1500px; height: 300px;">
                <img src="<%= boughts[cnt].getPath() %>" class="card-img-left" alt="..." style="width: 38%; height: 100%; position: absolute; left: 0; top: 0;">
                <div class="card-body" style="width: 60%; position: absolute; right: 0; top: 0;">
                    <h5 class="card-title"><%= boughts[cnt].getName() %></h5>
                    <p class="card-text"><%= boughts[cnt].getNote() %></p>
                </div>
                <div>
                    <p class="card-text" style="position: absolute; bottom: 55px; right: 20px;">购买时间: <%= boughts[cnt].getTime() %></p>
                </div>
                <form action="ShareServlet" method="post">
                    <input type="hidden" name="name" value="<%= boughts[cnt].getName() %>">
                    <input type="hidden" name="time" value="<%= boughts[cnt].getTime() %>">
                    <button type="submit" class="btn btn-primary" style="position: absolute; bottom: 10px; right: 10px;">分享商品</button>
                </form>
            </div>
